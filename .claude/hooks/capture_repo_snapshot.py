#!/usr/bin/env python3
"""
Hook to capture repository snapshots and git metrics.
...
def main():
    try:
        ...
        if event_type == "start":
            ...
            if success:
                print(f"✓ Created start snapshot zip for {model_lane}")
            else:
                print(f"✗ Failed to create start snapshot zip for {model_lane}", file=sys.stderr)
            ...
            if success:
                print(f"✓ Created end snapshot zip for {model_lane}")
            
            # Get base commit from session start log
            base_commit = get_base_commit_from_session_log(experiment_root, model_lane, session_id)
            
            if not base_commit:
                print(f"⚠ Could not find base commit for {model_lane}, skipping git diff and metrics", file=sys.stderr)
                return
            ...
            if diff_success:
                print(f"✓ Generated git diff for {model_lane} from {base_commit[:8]}")
