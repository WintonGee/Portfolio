...
// Example of how to integrate with Google Gemini API:
/*
import { GoogleGenerativeAI } from '@google/generative-ai'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create prompt with context
    const prompt = `
      You are an AI assistant representing a portfolio website. 
      Use the following context about the portfolio owner to answer questions:
      
      ${context}
      
      User question: ${message}
      
      Please provide a helpful and accurate response based on the portfolio information.
    `
...
async function getRelevantContext(message: string): Promise<string> {
  ...
  return "Portfolio context based on user query..."
}
*/
